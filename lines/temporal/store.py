...
class BetFundTemporalClient(object):  # pragma: no cover
    ...
    def __init__(self):
        ...
        self.loc = os.path.expanduser(
            "betfund/temporal/testDatastore/test.json"
        )
        self.db = self.load()
    ...
    def put_temporal(self, record: dict):
        ...
        if self._exist(key=record.get("key")):
            try:
                ...
                _LOGGER.info(
                    "PUT: UPDATE - eventId: {}".format(
                        record["key"].get("eventId")
                    )
                )
    ...
    def _exist(self, key: dict) -> bool:
        ...
        if not isinstance(key, dict):
            raise ValueError(
                "The provided key is invalid: {}".format(
